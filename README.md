...
把 QMT 内置策略环境暴露成一个仅本机可访问的 HTTP API，让外部 Python、浏览器、研究脚本或其他策略进程通过统一协议访问账户、持仓、委托、成交和行情，并在显式开启后发送限价委托/撤单。
...
打开 lite/qmt_http_bridge_lite.py，复制进 QMT 策略编辑器并运行。

QMT 控制台会输出监听地址和一次性随机 Token。

打开 lite/test.html。

填写 Token、账户号，调用 account / positions / quotes 等操作。
